...
- `index.html`에 Kakao Maps JS SDK 연동(REQ-29) — Day1 조별 동선 카드에 지도 표시, 실제 동작 확인 완료(`file://`에서도 뜸 — 도메인 제한보다 Kakao 콘솔의 "카카오맵" 제품 활성화 여부가 실질적 게이트였음). `ROUTE_DATA`의 좌표 중 `verified:false`인 지점은 지하철역 등 공공정보 기준 추정치라 실제 운영 전 재확인 필요.
