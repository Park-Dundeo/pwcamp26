...
- **교사 탭 PIN 잠금 (REQ-45):** 조가 선택·잠긴 학생 기기(`localStorage.pwcamp_team` + `pwcamp_unlocked_{team}==='1'`)에서는 4탭 앱 셸의 "교사" 탭이 잠김. `RESET_MASTER_PW_HASH`(mission.html의 기기 초기화 비밀번호와 동일값, 교사/개발자 전용)를 재사용해서 PIN 확인. 이 상수+게이트 로직이 `mission.html`/`map.html`/`board.html`/`index.html` 4곳에 중복 — 비밀번호 바꿀 때 4곳 다 수정할 것. `index.html`은 URL 직접 접근도 로드 시점에 오버레이로 막음.
- **교사 로그아웃 + 개별 이름 신원 (REQ-46):** `sessionStorage.pw_teacher_unlocked`/`pw_teacher_name`이 "교사 탭 게이트 통과 여부"와 "board.html 선생님 신원"을 동시에 나타내는 공유 상태임 — 한쪽에서 인증하면 다른 쪽도 자동 인식됨. 로그아웃은 `index.html` 우상단 배지(`#teacherSessionBadge`) 또는 `board.html`의 기존 `.who-badge`(교사/조장/관리자 통합)에서. `RESET_MASTER_PW_HASH`를 바꾸면 mission/map/board/index 4개 파일 + board.html의 "선생님" 인증 모달까지 전부 갱신할 것. 관리자(공지 전용, `ADMIN_PW_HASH`)와 교사(개인 이름, `RESET_MASTER_PW_HASH`)는 서로 다른 비밀번호로 완전히 분리된 신원임 — 절대 혼동하지 말 것. `map.html`/`board.html`에는 더 이상 게이트를 안 거치는 "← 대시보드" 링크가 없음(탭바의 "교사" 탭 하나로 통일) — 새 학생용 페이지에 대시보드 링크를 넣을 땐 반드시 탭바 경유로만 할 것.
