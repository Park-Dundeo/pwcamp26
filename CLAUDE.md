...
- **지도·교사 탭 기본 잠금 + 교사 명단 로그인 (REQ-47, REQ-45/46을 대체):** `sessionStorage.pw_teacher_unlocked==='1'` 여부만으로 잠금을 판단(과거 REQ-45의 "조 잠긴 기기일 때만" 조건은 폐기 — 이제 **항상** 잠금, 로그인해야 해제). 로그인은 공용 PIN이 아니라 **`TEACHERS` 명단(이름→PIN 해시)** 기반 — 이름을 먼저 고르고 그 사람의 PIN을 입력해야 함. `TEACHERS` 객체가 `mission.html`/`map.html`/`board.html`/`index.html` 4곳에 중복 — **교사 추가/PIN 변경 시 4곳 모두 수정할 것**. 현재는 예시 3명(박준서/1111, 김한솔/2222, 유성규/3333)만 등록돼 있고 **실제 명단은 추후 전달받는 대로 교체해야 함** — 운영 전 필수 확인 사항.
