...
- **지도·교사 탭 기본 잠금 + 교사 명단 로그인 (REQ-47~50, REQ-45/46을 대체):** `sessionStorage.pw_teacher_unlocked==='1'` 여부만으로 잠금을 판단(과거 REQ-45의 "조 잠긴 기기일 때만" 조건은 폐기 — 이제 **항상** 잠금, 로그인해야 해제). 로그인은 **이름 직접 입력(순수 텍스트, 자동완성 없음) + 본인 핸드폰 뒷자리 4자리** — `TEACHERS` 명단(이름→뒷자리4 SHA-256 해시)에서 이름+뒷자리 쌍이 일치해야 통과. `TEACHERS` 객체(현재 교사·헬퍼 25명 실명단)가 `mission.html`/`map.html`/`board.html`/`index.html` 4곳에 중복 — **명단/번호 변경 시 4곳 모두 수정할 것**.
  - **⚠️ 로그인 화면은 "아무 힌트도 주지 않는다" 원칙을 반드시 지킬 것** — 두 번 지적받은 이력 있음: (REQ-49) 이름 입력창에 `<datalist>` 자동완성을 붙였다가 미인증 상태에서도 교사 명단 25명이 그대로 노출되는 구멍이었음. (REQ-50) 안내문구·플레이스홀더에 "핸드폰 뒷자리 4자리"라고 명시했다가, 그 문구 자체가 "비밀번호는 전화번호 끝자리"라는 힌트라서 공격 범위를 대폭 좁혀준다는 지적을 받음. 지금은 화면 텍스트가 전부 "이름/비밀번호"로만 되어 있고, **실제 메커니즘(전화번호 뒷자리 사용)은 코드 주석·이 문서·REQUIREMENTS.md 같은 개발자 전용 문서에만 남겨두고 사용자 화면에는 절대 다시 노출하지 말 것.**
  - 잠금 대상: 🗺 지도(map.html)·👥 교사(index.html) — `mission.html`/`board.html`은 탭 클릭을 `teacherGatedClick()`으로 가로채고, `map.html`/`index.html`은 스스로 로드 시점에 자체 게이트를 검사(URL 직접 접근 차단). 🎯 미션·💬 소통방은 항상 열려있음(학생 기본 화면).
  - `sessionStorage.pw_teacher_unlocked`/`pw_teacher_name`은 "탭 게이트 통과 여부"와 "board.html 선생님 채팅 신원"을 동시에 나타내는 공유 상태 — 4개 파일 아무 데서나 로그인하면 나머지 전부 즉시 동기화됨(재인증 불필요).
  - 로그아웃: `mission.html`(`#statusBadge`, 미로그인 시 "🎓 학생 화면"으로 전환), `map.html`/`index.html`(`#teacherSessionBadge`), `board.html`(기존 `.who-badge`, 미인증 시에도 "🎓 학생 화면"을 항상 표시하도록 변경).
  - 관리자(공지 전용, `ADMIN_PW_HASH`)와 교사(개인별 이름+PIN, `TEACHERS`)는 완전히 분리된 신원 — 절대 혼동하지 말 것. `RESET_MASTER_PW_HASH`는 mission.html의 "기기 초기화" 기능 전용으로만 남음(교사 로그인과는 이제 무관).
  - `map.html`/`board.html`에는 게이트를 안 거치는 "← 대시보드" 링크가 없음(탭바의 "교사" 탭 하나로 통일) — 새 학생용 페이지에 대시보드 링크를 넣을 땐 반드시 탭바 경유로만 할 것.
...
- **2026-07-09 견학지 재편 (REQ-37, REQ-38), 2026-07-11 5조 경유지 교체 (REQ-51):** 견학지가 3곳→2곳으로 축소됨(연세대 언더우드가 기념관 보류). 현재 배정: **1·2·3조 = 전쟁기념관(경유지 각각 노들섬/이촌한강공원/서울로7017), 4·5·6조 = 서대문형무소(경유지 각각 청계천/경희궁/덕수궁 돌담길)**. 5조는 원래 광화문광장이었으나 7/17 광화문 인근 시위 예상으로 경희궁으로 교체됨. 전쟁기념관행은 대화에서 "전쟁 1조/2조/3조"로 표기된 걸 캠프 팀번호 1/2/3조로 그대로 해석한 것이라 확인 필요. 나머지 경유지도 구체 지정 없이 자체 선정한 것이라 실제 답사 후 교체 가능성 있음.
