...
- **지도·교사 탭 기본 잠금 + 교사 명단 로그인 (REQ-47~49, REQ-45/46을 대체):** `sessionStorage.pw_teacher_unlocked==='1'` 여부만으로 잠금을 판단(과거 REQ-45의 "조 잠긴 기기일 때만" 조건은 폐기 — 이제 **항상** 잠금, 로그인해야 해제). 로그인은 **이름 직접 입력(순수 텍스트, 자동완성 없음) + 본인 핸드폰 뒷자리 4자리** — `TEACHERS` 명단(이름→뒷자리4 SHA-256 해시)에서 이름+뒷자리 쌍이 일치해야 통과. `TEACHERS` 객체(현재 교사·헬퍼 25명 실명단)가 `mission.html`/`map.html`/`board.html`/`index.html` 4곳에 중복 — **명단/번호 변경 시 4곳 모두 수정할 것**. **⚠️ 이름 입력창에 `<datalist>`나 그 밖의 자동완성/제안 목록을 절대 붙이지 말 것** — REQ-49에서 한 번 붙였다가 미인증 상태에서도 교사 명단 전체가 노출되는 구멍이었음이 지적되어 제거함. 로그인 폼은 "아무 힌트도 주지 않는다"는 원칙을 지킬 것(오류 메시지도 이름/번호 중 어느 쪽이 틀렸는지 특정하지 않음).
