...
- **지도·교사 탭 기본 잠금 + 교사 명단 로그인 (REQ-47/48, REQ-45/46을 대체):** `sessionStorage.pw_teacher_unlocked==='1'` 여부만으로 잠금을 판단(과거 REQ-45의 "조 잠긴 기기일 때만" 조건은 폐기 — 이제 **항상** 잠금, 로그인해야 해제). 로그인은 **이름 직접 입력(`<datalist>` 자동완성) + 본인 핸드폰 뒷자리 4자리** — `TEACHERS` 명단(이름→뒷자리4 SHA-256 해시)에서 이름+뒷자리 쌍이 일치해야 통과(버튼 선택 방식 아님). `TEACHERS` 객체(현재 교사·헬퍼 25명 실명단)가 `mission.html`/`map.html`/`board.html`/`index.html` 4곳에 중복 — **명단/번호 변경 시 4곳 모두 수정할 것**. `<datalist id="teacherNameList">`는 각 파일 로드 시 `TEACHERS` 키로 자동 채워짐(하드코딩 이중관리 아님, 값만 갱신하면 자동완성도 같이 갱신됨).
